...
public class GraphPanel extends JPanel implements MouseListener, MouseMotionListener {
	...
	public void mouseClicked(MouseEvent e) {
			...
			if(e.isControlDown() && e.isShiftDown()){
				...
				if(SwingUtilities.isLeftMouseButton(e)){
					if(!graph.isDestination(selected))
						...
						JOptionPane.showMessageDialog(null, "Hedef kaynak olarak ayarlanamaz");
				} else if(SwingUtilities.isRightMouseButton(e)) {
					if(!graph.isSource(selected))
						graph.setDestination(selected);
					else
						JOptionPane.showMessageDialog(null, "Kaynak, Hedef olarak ayarlanamaz");
				}else
					return;
		...
		if(hoveredEdge!=null){
			...
			String input = JOptionPane.showInputDialog("Için aðýrlýðý girin " + hoveredEdge.toString()
			+ " : ");
			try {
				...
				if (weight > 0) {
					...
					JOptionPane.showMessageDialog(null, "Aðýrlýk pozitif olmalý");
				}
		...
		for(Node node : graph.getNodes()) {
			if(DrawUtils.isOverlapping(e, node.getCoord())){
				JOptionPane.showMessageDialog(null, "Örtüþen Düðüm oluþturulamýyor");
				return;
			}
		}
